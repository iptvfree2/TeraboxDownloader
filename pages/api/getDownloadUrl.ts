...
export default async function handler(
    ...
    try {
        ...
        var response = await axiosClient.post(`https://www.terabox.com/share/download`+`?app_id=${appId}`+`&web=1`+`&channel=dubox`+`&clienttype=0`+`&jsToken=${jsToken}`+
        `&dp-logid=${dpLogId}`+`&shareid=${shareId}`+`&sign=${sign}`+`&timestamp=${timestamp}`,
